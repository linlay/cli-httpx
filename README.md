...
`httpx` 是一个面向智能体和自动化脚本的 HTTP CLI，用来把“登录、带状态请求、提取响应字段”固化进 site 配置。
...
1. 写一个 `demo.toml`
2. 设置环境变量
3. 执行 `httpx --config . login demo`
4. 执行 `httpx --config . run demo me`
5. 执行 `httpx --config . sites` 或 `httpx --config . actions demo` 查看渐进披露信息
...
httpx state nexus
httpx login nexus
httpx run nexus profile
httpx inspect nexus profile
httpx version
httpx --version
```
...
```bash
httpx --config ./examples --format json run demo me
httpx run demo --format json --config ./examples me
```
...
```bash
./httpx --format json login nexus
./httpx --format json run nexus profile
./httpx site nexus
./httpx actions nexus
```
...
```bash
./httpx --state "$HOME/.local/httpx-state" --format json login nexus
./httpx --state "$HOME/.local/httpx-state" --format json run nexus profile
./httpx --state "$HOME/.local/httpx-state" state nexus
```
